...
class OracleCDCService:
    ...
    def collect_new_calls(self) -> List[str]:
        ...
        try:
            ...
            query = f"""
                SELECT DISTINCT CALL_ID, CALL_TIME
                FROM {SOURCE_TABLE}
                WHERE CALL_TIME > SYSDATE - (120 / 1440)
                AND CALL_ID NOT IN (
                    SELECT CALL_ID FROM CDC_PROCESSED_CALLS where TEXT_TIME > SYSDATE - (120 / 1440)
                )
    ...
    def receive_ml_results(self):
        ...
        try:
            response = self.sqs_client.receive_message(
                QueueUrl=SQS_INBOUND_QUEUE_URL,  # Receive results from inbound queue
            ...
            for message in messages:
                try:
                    ...
                    if msg_type == MESSAGE_TYPES['ML_RESULT']:
                        ...
                        if success:
                            # Delete message from inbound queue
                            self.sqs_client.delete_message(
                                QueueUrl=SQS_INBOUND_QUEUE_URL,  # Delete from inbound queue
    ...
    def write_ml_result(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.oracle_conn.commit()
                        ...
                        # Also save to CONVERSATION_SUMMARY
            # try:
            #     cursor.execute("""
            #         INSERT INTO CONVERSATION_SUMMARY (source_id, text, source_type, creation_date)
            #         VALUES (:source_id, :text, :source_type, SYSDATE)
            #     """, {
            #         'source_id': call_id,
            #         'text': summary_text,
            #         'source_type': 'CALL'
            ...
            # # Also save to CONVERSATION_CATEGORY
            # try:
            #     cursor.execute("""
            ...
            #     })
            #     self.oracle_conn.commit()
            #     oracle_logger.info(f"✅ Conversation category written: {call_id}")
            # except Exception as e:
